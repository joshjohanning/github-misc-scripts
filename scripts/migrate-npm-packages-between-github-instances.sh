...
# Usage: ./migrate-nuget-packages-between-orgs.sh <source-org> <source-host> <target-org> <target-host>
#
#
# Prereqs:
...
if [ -z "$GH_SOURCE_PAT" ]; then
    echo "define GH_SOURCE_PAT env var"
    exit 1
fi

if [ -z "$GH_TARGET_PAT" ]; then
    echo "define GH_TARGET_PAT env var"
    exit 1
fi
...
# log into source
gh auth login -h $SOURCE_HOST --with-token <<< $GH_SOURCE_PAT
...
echo "$packages" | while IFS= read -r response; do
  ...
  versions=$(GH_HOST="$SOURCE_HOST" gh api "/orgs/$SOURCE_ORG/packages/npm/$package_name/versions" -q '.[] | .name')
